...
- ILib 文件头 56 字节：Title(40) + VerFlag(4) + ImageCount(4) + ColorCount(4) + PaletteSize(4) + Palette(1024)
- `btVersion` 由 VerFlag 决定（!=0 → 12字节图像头含 bits 字段，==0 → 8字节）
- `colorCount <= 256`：8-bit 调色板索引，Palette[0] = 透明
- `colorCount > 256`：16-bit RGB565 直接颜色（R5 G6 B5），0x0000 = 透明
- WIX 文件名兼容：标准路径找不到时，遍历同目录 `*.wix` 按 base name 匹配（处理 `Deco..wix` 等异常命名）
...
**16-bit RGB565 直接颜色**（`colorCount > 256`）：
```go
for j := 0; j < w*h; j++ {
    c16 := uint16(pixelBytes[j*2]) | uint16(pixelBytes[j*2+1])<<8
    rgba.Pix[j*4+0] = uint8((c16 >> 11) << 3)       // R5
    rgba.Pix[j*4+1] = uint8(((c16 >> 5) & 0x3F) << 2) // G6
    rgba.Pix[j*4+2] = uint8((c16 & 0x1F) << 3)       // B5
    rgba.Pix[j*4+3] = 255
    if c16 == 0 { rgba.Pix[j*4+3] = 0 }              // 0x0000 = transparent
}
```
