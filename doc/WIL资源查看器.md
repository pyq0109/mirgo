...
| 40-43 | 4字节 | VerFlag | 版本标志（==0 时图像头为12字节，!=0 时为8字节） |
...
**网格滚动**：点击缩略图不触发自动滚动（仅导航按钮和方向键设置 `GridScrollTo`）。滚动目标行高使用 `ig.FrameHeight() + ig.CurrentStyle().ItemSpacing().Y` 计算 ImGui 实际行高，而非固定常量。

**键盘输入**：方向键通过 GLFW key callback 事件驱动（`SetGLFWCallbacks` 的 `KeyHandler` 参数），只响应 `Press` 事件，不响应 `Repeat`，避免长按过快跳转。
...
**实际验证结果**：所有游戏客户端 WIL 文件均为 ILib 格式（`#ILIB`/`#INDX` 签名），`VerFlag=0` 映射到 `BtVersion=0`（12 字节图像头）。8-bit 文件（如 Hum.wil、Mon1.WIL）`ColorCount=256`，16-bit 文件（如 Items.wil、DnItems.wil）`ColorCount=65536`。
